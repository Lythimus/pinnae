...
**`detector.py`** — `BandDetector` maintains a rolling per-bin noise floor: `NOISE_HISTORY_CHUNKS` (700, ~30 s) of per-chunk band-power arrays, floored via `np.median` across that window *per FFT bin* (not a single scalar band median) — a persistent narrowband tone is absorbed into its own bin's floor and stops firing, while a call at a neighboring bin is unaffected. Onset/offset is a simple state machine: a call opens when peak band power exceeds `DETECTION_THRESHOLD_DB` above the floor, **and** passes the call-shape gate, and closes after `CALL_END_SILENCE_MS` of silence. Call `flush()` before closing the stream to capture any in-progress call. `compute_power_spectrum` is a module-level function used by `listen.py`; it is not a method of `BandDetector`.
...
**Offender-bin notch + FM gate** (electronic-ultrasonic false positives, distinct from the music-distortion call-shape gate above) — the per-bin floor already absorbs *steady* electronic tones; the two mechanisms below target *intermittent narrowband* transients (switching-supply/PWM/relay chatter) that spike faster than the floor adapts and pass the shape gate because they're narrowband, not broadband. Both are **implemented but ship inert** pending a real electronics-only capture (see below) — `config.NOISE_NOTCH_HZ` is `[]` and `config.FM_GATE_ENABLED` is `False`.
- **Static notch** (`NOISE_NOTCH_ENABLED` / `NOISE_NOTCH_HZ` in `config.py`) — a list of `(f_lo, f_hi)` Hz windows excluded from the peak search in `BandDetector.process()` (set to `-inf` before `argmax`), computed once per instance in `__init__` via `_bins_in_notch()`. Applies in **every** band including alarm, since it targets specific known-bad bins rather than a class of signal shape.
- **FM gate** (`FM_GATE_ENABLED` / `MIN_FREQ_MODULATION_HZ` / `FM_GATE_BANDS` in `config.py`) — rat social USVs are frequency-modulated (they glide); electronic tones sit at a fixed frequency. `BandDetector` tracks each call's peak-frequency min/max while active; `_close_call()` computes `freq_span_hz = max − min` and rejects (returns `None`, no event emitted) when `self.band_name in FM_GATE_BANDS` and `freq_span_hz < MIN_FREQ_MODULATION_HZ`. **Deliberately excludes `"alarm"` by default** — 22 kHz alarm calls are themselves near-constant-frequency, so this gate would delete real ones there if enabled for that band. `DetectionEvent.freq_span_hz` is populated for every event (even when the gate is off) so `evaluate.py` can report the real pos-vs-neg distribution before a threshold is chosen.
- To validate and populate these: `python listen.py record --out electronics_neg.wav --duration 300` (electronics on, no rat) on the deployed rig, then `python evaluate.py --neg electronics_neg.wav --pos <alarm/social corpora> --gate off` to read off offender frequency clusters and `freq_span_hz` percentiles, then `--sweep-fm` to choose `MIN_FREQ_MODULATION_HZ`.
...
**`evaluate.py`** — standalone offline harness that replays labeled WAV corpora through the real `compute_power_spectrum` + `BandDetector` pipeline to measure and tune the call-shape gate and the offender-notch/FM gate. Resamples every input file to `config.SAMPLE_RATE` via `scipy.signal.resample_poly` (eval-only dependency, not in `requirements.txt`'s runtime set). `--gate on|off` toggles `CALL_SHAPE_GATE_ENABLED` for a single-run report of per-file/per-band detection counts, and now also prints `freq_span_hz` percentiles per band for the *closed events themselves* (`print_freq_span_summary`), independent of gate state. `--sweep` grid-searches `MAX_CALL_BAND_FRACTION` × `MIN_PEAK_CONCENTRATION_DB` and reports positive-recall-kept vs. negative-rejected fractions relative to a gate-off baseline, plus prints raw shape-feature percentiles (pos vs. neg) so thresholds are chosen from data. `--sweep-fm` runs the analogous sweep for the electronics defenses: `MIN_FREQ_MODULATION_HZ` (from `--sweep-fm-thresholds`) crossed with notch on/off, relative to a gates-off baseline (`sweep_fm()`).

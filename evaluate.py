...
def report_counts(files: list[tuple[Path, np.ndarray]], band_names: list[str], label: str) -> dict:
    ...
    Returns ({path: {band: n_events}}, diagnostic_records, {band: [DetectionEvent, ...]}, fm_records).
    """
    result = {}
    all_records = []
    ...
    return result, all_records, all_events, all_fm_records
...
def main() -> None:
    ...
    if not args.sweep:
        ...
        print(f"\n=== gate={args.gate} ===")
        print("\n-- positive files --")
        _, pos_records, pos_events, pos_fm_records = report_counts(pos_files, band_names, "pos")
        print("\n-- negative files --")
        _, neg_records, neg_events, neg_fm_records = report_counts(neg_files, band_names, "neg")
    ...
    print("\n=== baseline (gate off) ===")
    pos_baseline, pos_records, _, pos_fm_records = report_counts(pos_files, band_names, "pos")
    neg_baseline, neg_records, _, neg_fm_records = report_counts(neg_files, band_names, "neg")
...
def sweep_fm(
    ...
    pos_baseline, pos_records, pos_events, pos_fm_records = report_counts(pos_files, band_names, "pos")
    neg_baseline, neg_records, neg_events, neg_fm_records = report_counts(neg_files, band_names, "neg")
